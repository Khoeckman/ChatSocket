...
const JSBackedPropertyValue = (config, propName, isIntegerDesired, isFloatDesired) => ({
  ...
  getValue() {
    if (isIntegerDesired) return new java.lang.Integer(config[propName])
    if (isFloatDesired) return new java.lang.Float(config[propName])
...
const DecoratorCollector = config => ({
  collectProperties(instance) {
    ...
    Object.getOwnPropertyNames(clazz.__config_props__).forEach(prop => {
      ...
      const isIntegerDesired =
        attributes.type === PropertyType.NUMBER || attributes.type === PropertyType.SLIDER || attributes.type === PropertyType.SELECTOR
